...
- **Voortgang van een project wordt afgeleid, nooit opgeslagen**: `packages/shared/calc/projectvoortgang.ts` is de enige plek die bepaalt wat "gemaakt", "geleverd" en "gefactureerd" betekenen — web én API rekenen ermee. De vier toestanden zijn een **opdeling** van het bestelde aantal (`gefactureerd · geleverd nog niet gefactureerd · klaar op de vloer · nog te maken`), dus ze tellen altijd op tot besteld. Een gecrediteerd stuk is éérst gefactureerd en telt dus **één** keer; het apart optellen maakte van 100 stuks er 102
- **Deelleveringen zijn de regel, niet de uitzondering**: `paklijsten[]` en `facturen[]` (meervoud). Een pakbon gaat over wat er op dát moment klaarligt, een factuur over wat er geleverd en nog niet gefactureerd is. Een creditnota is geen negatieve factuur maar een eigen document met eigen nummerreeks (`CRED-`) dat de gecrediteerde factuur laat staan
- **Een order kan deels gereed zijn** (`aantalGereed` op `ProductieOrder`), ingevuld bij het gereedmelden. Afleiden uit de tijdregistratie zou 0 opleveren zodra iemand vergeet te klokken, en dan lijkt een volle kist leeg
- **Offerteversies delen één documentnummer** (`documentNr`): een nieuwe versie vervángt de vorige. Het id kan dat niet zijn — dat is de primary key. Bestaande offertes houden hun eigen nummer; die zijn zo de deur uit gegaan
